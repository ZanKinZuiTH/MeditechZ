...
{
   public class EcountItemIssueViewModel : MediTechViewModelBase
    {
        #region Properties
        ...
        public EcountItemIssueViewModel()
        {
        ...
        private void Save()
        {
            try
            {
                ...
                DataService.Inventory.ManageItemIssueEount(model, AppUtil.Current.UserID);
                SaveSuccessDialog();
        ...
        private void ImportFile()
        {
            ...
            int pgBarCounter = 0;
            // TotalRecord = 0;
            EcountItemIssue view = (EcountItemIssue)this.View;
            try
            {
                if (FileLocation.Trim() != string.Empty)
                {
                    ...
                    foreach (DataRow drow in ImportData.Rows)
                    {
                        ...
                        newRow.SelectItemMaster = ItemMasters.Where(p => p.ItemMasterUID == itemModel.ItemMasterUID && p.SerialNumber == newRow.SerialNumber).FirstOrDefault();
                           // .OrderBy(p => p.ExpiryDttm).FirstOrDefault();
                        newRow.IMUOMUID = itemModel.IMUOMUID;
                        if (newRow.SelectItemMaster == null)
                        {
                            WarningDialog("ไม่มี " +itemModel.Name + " ในคลัง");
                            continue;
                        }
